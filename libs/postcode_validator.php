...
class Postcode_Validator
{
     ...
     */
    public function isValid($countryCode='PL', $postalCode = '00-999')
    {
        if (!isset($this->formats[$countryCode])) {
            return false;
        }

        if (empty($this->formats[$countryCode])) {
            return false;
        }

       
  
        if (preg_match("/^{$this->formats[$countryCode]}$/", trim($postalCode), $match)) {
            return true;
        }
...
function checkPostcode($postalCode, $countryCode)
{
    ...
    $validator = new Postcode_Validator();

    $result = $validator->isValid($countryCode, $postalCode);
